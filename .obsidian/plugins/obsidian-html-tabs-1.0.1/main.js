...
function render(tabs, source, container, ctx) {
  if (!tabs.hasTabs()) {
    renderCodeBlock(container, source, "html");
    return;
  }
  const plugin2 = window.html_tabs_plugin;
  ...
  for (let index = 0; index < tabs.tabs.length; index++) {
    ...
    import_obsidian.MarkdownRenderer.render(plugin2.app, element.label, divTab, ctx.sourcePath, plugin2);
  }
  ...
  for (let index = 0; index < tabs.tabs.length; index++) {
    ...
    import_obsidian.MarkdownRenderer.render(plugin2.app, element.content, divTabContent, ctx.sourcePath, plugin2);
  }
}
...
// src/main.ts
module_default.prefix("data-x-");
module_default.start();
var HTMLTabsPlugin = class extends import_obsidian2.Plugin {
  ...
  async onload() {
    ...
    console.log(`Alpine.js ${module_default.version} loaded`);
  }
  onunload() {
    console.log(`Alpine.js ${module_default.version} unloaded`);
  }
  // async loadSettings() {
  ...
  registerCodeblockPostProcessorWithPriority(language, priority, processor) {
    const registered = this.registerMarkdownCodeBlockProcessor(language, processor);
    registered.sortOrder = priority;
  }
  async renderTabs(source, el, ctx) {
